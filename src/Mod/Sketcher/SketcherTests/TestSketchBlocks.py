...
class TestSketchBlocks(unittest.TestCase):
    ...
    def testReloadNestedGroup(self):
        index = self.insert()
        s = self.sketch
        group = s.Constraints[index]
        outer = s.addGeometry(Part.LineSegment(App.Vector(20, 0, 0), App.Vector(20, 10, 0)), True)
        s.addConstraint(Sketcher.Constraint("Group", [outer, 0, group.First, 0]))
        self.assertEqual(s.solve(), 0)
        self.write(True)
        SketcherBlock.reload(s, index)
        self.assertEqual(s.solve(), 0)
        self.assertEqual(sum(c.Type == "Group" for c in s.Constraints), 2)
        parent = s.Constraints[1]
        s.addConstraint(Sketcher.Constraint("Distance", parent.First, 20.0))
        self.assertEqual(s.solve(), 0)
        self.assertAlmostEqual(s.Geometry[s.Constraints[0].Second].Radius, 10, places=5)
