...
void SoBrepEdgeSet::renderBase(SoGLRenderAction* action)
{
    ...
    if (patternsDirty) {
        ...
        for (int i = 0; i <= coordIndex.getNum(); ++i) {
            ...
            if (i > start) {
                const int pattern = edge < linePatterns.getNum() ? linePatterns[edge] : -1;
                const float width = edge < lineWidths.getNum() && lineWidths[edge] > 0
                ...
                batch.materials.push_back(edge);
            }
            start = i + 1;
            ++edge;
        }
