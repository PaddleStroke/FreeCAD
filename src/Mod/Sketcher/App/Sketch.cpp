    ...
    for (size_t i = 0; i < groups.size(); i++) {
        ...
        for (size_t j = 0; j < groups[i].size(); j++) {
            Base::Console().warning(
                ...
                *param2geoelement.find(*std::next(groups[i].begin(), j))->first,
                std::get<0>(param2geoelement.find(*std::next(groups[i].begin(), j))->second),
                (int)std::get<1>(param2geoelement.find(*std::next(groups[i].begin(), j))->second)
            );
        }
    }
#endif  // DEBUG_BLOCK_CONSTRAINT
...
#ifdef DEBUG_BLOCK_CONSTRAINT
                Base::Console().warning("\nTentatively blocking group %d, with param=%f", i, *thisparam);
#endif  // DEBUG_BLOCK_CONSTRAINT
                break;
            }
        }
        ...
        case Group: {
            ...
            if (handle < 0 || handle >= static_cast<int>(Geoms.size())
                || !(
                    ...
                    || (constraint->Type == Group && Geoms[handle].geo->is<Part::GeomPoint>())
                )) {
                return -1;
            }
