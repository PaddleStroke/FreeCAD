...
CustomTitleBar::CustomTitleBar(QMainWindow* mainWindow)
    : QWidget(mainWindow), m_mainWindow(mainWindow) {
...
void CustomTitleBar::updateMaximizeRestoreIcon() {
    ...
    if (m_mainWindow->isMaximized()) {
        m_maximizeButton->setIcon(Gui::BitmapFactory().iconFromTheme(isDarkTheme ? "window-restore-dark" : "window-restore"));
    }
    else {
        m_maximizeButton->setIcon(Gui::BitmapFactory().iconFromTheme(isDarkTheme ? "window-maximize-dark" : "window-maximize"));
    }
}
