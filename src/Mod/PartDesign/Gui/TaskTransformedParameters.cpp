...
void TaskTransformedParameters::setupUI()
{
    ...
    connect(ui->listWidgetFeatures, &QListWidget::itemChanged,
        this, &TaskTransformedParameters::onFeatureItemChanged);
    ...
    connect(ui->groupFeatureList, &QGroupBox::toggled,
        this, &TaskTransformedParameters::onGroupFeaturesToggled);
...
void TaskTransformedParameters::populateFeatureList()
{
    ...
    for (App::DocumentObject* obj : body->Group.getValues()) {
        if (!obj) continue;
        if (obj == pcTransformed) break; // Stop at self
