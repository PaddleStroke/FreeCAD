...
IFS=$'\n'
dylibs=($(find ${CONTAINING_FOLDER}/AstoCAD.app -name "*.dylib"))
shared_objects=($(find "${CONTAINING_FOLDER}/AstoCAD.app" -name "*.so"))
executables=($(find "${CONTAINING_FOLDER}/AstoCAD.app" -type f -perm +111 -exec file {} + | grep "Mach-O 64-bit executable" | sed 's/:.*//g'))
IFS=$' \t\n' # The default
...
run_codesign "${CONTAINING_FOLDER}/AstoCAD.app/Contents/packages.txt"
run_codesign "${CONTAINING_FOLDER}/AstoCAD.app/Contents/Library/QuickLook/QuicklookFCStd.qlgenerator/Contents/MacOS/QuicklookFCStd"

# Finally, sign the app itself (must be done last)
run_codesign "${CONTAINING_FOLDER}/AstoCAD.app"

# Create a disk image from the folder
DMG_NAME="AstoCAD-${VERSION_MAJOR}.${VERSION_MINOR}.${VERSION_PATCH}-mac-${ARCH}.dmg"
echo "Creating disk image ${DMG_NAME}"
pip3 install "dmgbuild[badge_icons]>=1.6.0,<1.7.0"
dmgbuild -s dmg_settings.py -Dcontaining_folder="${CONTAINING_FOLDER}" "AstoCAD" "${DMG_NAME}.dmg"

# Submit it for notarization (requires that an App Store API Key has been set up in the notarytool)
xcrun notarytool submit --wait --keychain-profile "AstoCAD" ${DMG_NAME}
