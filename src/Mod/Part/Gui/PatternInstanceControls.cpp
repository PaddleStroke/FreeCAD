...
PatternInstanceControls::PatternInstanceControls(Gui::View3DInventorViewer* viewer,
                                                 QObject* parent)
    : QObject(parent)
{
    setViewer(viewer);
...
void PatternInstanceControls::setViewer(Gui::View3DInventorViewer* newViewer)
{
    ...
    if (viewer) {
        connect(viewer.data(),
                &Gui::View3DInventorViewer::cameraChanged,
                this,
                [this]() {
                    attachCameraSensor();
                    updatePositions();
...
void PatternInstanceControls::updatePositions()
{
    ...
    for (auto& info : buttons) {
        ...
        QPoint pxCoord = viewer->toQPoint(viewer->getPointOnViewport(
            SbVec3f(static_cast<float>(info.instance.center.x),
        ...
        if (pxCoord.x() < 0 || pxCoord.y() < 0
            || pxCoord.x() + buttonSize.width() > hostSize.width()
